...
import in.co.online.sport.model.UserModel;
  ...
  public class LoginCtl extends BaseCtl {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (id > 0) {
			...
			ServletUtility.redirect(OSSView.LOGIN_CTL, request, response);
			return;
		}
		if (session.getAttribute("user") != null) {
			ServletUtility.redirect(OSSView.WELCOME_CTL, request, response);
			return;
		}
		...
		if (OP_SIGN_IN.equalsIgnoreCase(op)) {
			...
			try {
				...
				if (bean != null) {
					...
					if(bean.getRoleId()==2) {
					Long cid=DataUtility.getLong(String.valueOf(session.getAttribute("cId")));
					if(cid>0) {
						ServletUtility.redirect(OSSView.CART_CTL+"?cIdd="+cid, request, response);
						return;
					}
					}
			...
			ServletUtility.redirect(OSSView.USER_REGISTRATION_CTL, request, response);
		return;
		}
